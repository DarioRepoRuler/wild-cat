...
def plot_prediction_GT(image, boxes, labels, ax=None, plot_prob=True):
    ...
    plot_results_GT(image[0].permute(1, 2, 0).detach().cpu().numpy(), boxes, labels, ax, plot_prob=plot_prob)
...
def generate_xml_annotation(annotate_folder, file, objects, h, w):
    ...
    for obj in objects:
        ...
        difficult.text = "0"
        numb = ET.SubElement(object, "numb")
        numb.text = str(obj["numb"])
...
def res2xml(score, labels, boxes, outputfolder, file_name, h, w):
    #top_k = len(score)
    objects =[]
    for i in range(len(score)):
        objects.append({"name": CLASSES[labels[i]], "numb": i ,"xmin": boxes[i][1].item(), "ymin": boxes[i][3].item(), "xmax": boxes[i][0].item(), "ymax": boxes[i][2].item()})
    #print(f"Objects: {objects}")
    annotate_folder = os.path.join(outputfolder, 'Annotations')
    generate_xml_annotation(annotate_folder, file_name, objects, h, w)